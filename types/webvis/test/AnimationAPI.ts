function test(testContext : webvis.ContextAPI) : void
{
    ...
    testContext.registerListener([webvis.EventType.ANIMATION_FRAMES_CREATED], (event : webvis.AnimationFramesCreatedEvent) => {
        console.log("Animation frames created", event.name, event.frames);
    });

    testContext.registerListener([webvis.EventType.ANIMATION_FRAMES_REMOVED], (event : webvis.AnimationFramesRemovedEvent) => {
