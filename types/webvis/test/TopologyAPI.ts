function test(testContext : webvis.ContextAPI) : void
{
    ...
    const arcDescriptor : webvis.TopologyDescriptor<webvis.TopologySubType.CIRCULAR_ARC> = testContext.createCircularArcDescriptor(
        [0, 0, 0],
        [1, 1, 1],
        [2, 2, 2],
    );
    ...
    const internalHandles : Promise<Array<webvis.TopologyHandle|undefined>> =  testContext.mapOriginalToInternalTopologyHandles(
        1,
        [origHandle0, origHandle1],
    );

    const origHandles : Promise<Array<webvis.OriginalTopologyHandle|undefined>> = testContext.mapInternalToOriginalTopologyHandles(
        [topoHandle0, topoHandle1],
    );

    const removeTopoFromSelectionPromise : Promise<void> = testContext.removeTopologyFromSelection(topoHandle0);
    const removeToposFromSelectionPromise : Promise<void> = testContext.removeTopologyFromSelection([topoHandle0, topoHandle1]);

    const topoDescriptor : Promise<webvis.TopologyDescriptor> = testContext.requestTopologyDescriptor(
        topoHandle0
    );

    const boxDescriptor : Promise<webvis.TopologyDescriptor<webvis.TopologySubType.BOX>> = testContext.requestBoxDescriptor(
        [2, 3, 4],
    );
