function test(testContext : webvis.ContextAPI) : void
{
    ...
    const removeSelectionResult : Promise<webvis.ChangeSelectionResult> = testContext.removeFromSelection([1,3,5], false);
    ...
    testContext.registerListener([webvis.EventType.SELECTION_CHANGED], (event : webvis.SelectionChangedEvent) => {
        console.log("Selection changed", event.targetNodeID, event.oldSelectionCount, event.newSelectionCount, event.selectedNodes);
    });
}
