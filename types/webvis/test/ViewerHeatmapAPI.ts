function test(testContext : webvis.ContextAPI, testViewer : webvis.ViewerAPI) : void
{
    ...
    testContext.registerListener([webvis.EventType.VIEWER_HEATMAP_CREATED], (event : webvis.ViewerHeatmapCreatedEvent) => {
        console.log("Heatmap created", event.heatmapId, event.properties);
    });

    testContext.registerListener([webvis.EventType.VIEWER_HEATMAP_CHANGED], (event : webvis.ViewerHeatmapChangedEvent) => {
        console.log("Heatmap changed", event.heatmapId, event.properties);
    });

    testContext.registerListener([webvis.EventType.VIEWER_HEATMAP_REMOVED], (event : webvis.ViewerHeatmapRemovedEvent) => {
        console.log("Heatmap removed", event.heatmapId);
    });

    testContext.registerListener([webvis.EventType.VIEWER_HEATMAP_CONFIG_CHANGED], (event : webvis.ViewerHeatmapConfigChangedEvent) => {
