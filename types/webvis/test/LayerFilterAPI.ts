function test(testContext : webvis.ContextAPI) : void
{
    ...
    testContext.registerListener([webvis.EventType.LAYERFILTER_REGISTERED], (event : webvis.LayerFilterRegisteredEvent) => {
        console.log("LayerFilter registered", event.name, event.enabled);
    });

    testContext.registerListener([webvis.EventType.LAYERFILTER_UNREGISTERED], (event : webvis.LayerFilterUnregisteredEvent) => {
